...
public class ExcelReaderWriterIntegrationTests
{
    ...
    public void WriteAndRead_SimpleRecords()
    {
        ...
        {
            new() { Name = "Alice", Age = 30, BirthDate = new DateTime(1994, 5, 15) },
            new() { Name = "Bob", Age = 25, BirthDate = new DateTime(1999, 8, 22) }
        };
        ...
        var results = reader.GetRecords<Person>().ToList();
    ...
    public void WriteAndRead_WithHeaders()
    {
        var products = new List<Product>
        {
            ...
            new() { Name = "Gadget", Price = 19.99m, InStock = false }
        };

        using var stream = new MemoryStream();
        using (var writer = new ExcelWriter(stream,
                   new ExcelConfiguration { HasHeaderRecord = true, HeaderRow = 1, StartRow = 2 }, true))
        {
        ...
        using var reader = new ExcelReader(stream,
            new ExcelConfiguration { HasHeaderRecord = true, HeaderRow = 1, StartRow = 2 });
    ...
    public void WriteAndRead_WithCustomMapping()
    {
        ...
        var people = new List<Person>
        {
            new() { Name = "Charlie", Age = 35, BirthDate = new DateTime(1989, 3, 10) }
        };

        using var stream = new MemoryStream();
        using (var writer = new ExcelWriter(stream, config, true))
        {
            writer.Context.RegisterClassMap(new PersonMap());
            writer.WriteRecords(people);
        }
    ...
    public void WriteAndRead_WithIgnoredProperty()
    {
        ...
        {
            new() { Name = "Alice", Age = 30, Secret = "TopSecret" }
        };
    ...
    public void WriteAndRead_Dates()
    {
        ...
        {
            new() { EventDate = new DateTime(2024, 1, 15), EventName = "Launch" }
        };
    ...
    public void WriteAndRead_MultiSheet()
    {
        ...
        using (var writer = new ExcelWriter(stream, writeConfig, true))
        {
            writer.WriteRecords(people);
        }
